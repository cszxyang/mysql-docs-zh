...
7. 有个教 innodb_autoextend_increment 的参数，当共享表空间变满时，会自动进行扩展，而这个参数就是用来指定扩容的增量，该参数不适用于每表文件表空间的文件，无论 innodb_autoextend_increment 设置成什么，该文件都将自动扩展。一开始这种扩展会小幅增长，此后每次扩展扩容 4MB。
